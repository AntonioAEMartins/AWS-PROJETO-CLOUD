...
resource "aws_autoscaling_policy" "load-balancer-policy" {
  name                      = "load-balancer-policy"
  autoscaling_group_name    = aws_autoscaling_group.asg.name
  policy_type               = "TargetTrackingScaling"
  estimated_instance_warmup = 260

  target_tracking_configuration {
    predefined_metric_specification {
      predefined_metric_type = "ALBRequestCountPerTarget"
      resource_label         = "${split("/", module.alb.load_balancer_id)[1]}/${split("/", module.alb.load_balancer_id)[2]}/${split("/", module.alb.load_balancer_id)[3]}/targetgroup/${split("/", module.alb.target_group_arn)[1]}/${split("/", module.alb.target_group_arn)[2]}"
    }
